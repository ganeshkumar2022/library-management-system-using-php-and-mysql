...
include('db.php');
$sql="update category set name='$name',status='$status',update_time='$date' where id=$id"; 
...
<div class="alert alert-success alert-dismissible">
  <button type="button" class="close" data-dismiss="alert">&times;</button>
  <strong>Error!</strong> Error to edit category.
</div>
    <?php
}
...
<div class="form-group">
    ...
    <input type="text" class="form-control" name="name" value="<?php echo $row["name"]; ?>" placeholder="Enter category name" id="email">
    <span id="ne" class="text-danger"></span>
</div>
<label>Status:</label>
<input type="radio" name="status" value="active" <?php if($row["status"]=="active") { echo "checked"; } ?> > Active
<input type="radio" name="status" value="inactive" <?php if($row["status"]=="inactive") { echo "checked"; } ?>> In active<br/>
